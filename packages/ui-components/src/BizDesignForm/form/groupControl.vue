<template>
  <div
    ...
    <template v-else-if="element.type === 'title'">
      <div class="title item-center" :class="[element.config?.className]" v-bind="element.control">
        <span style="display: inline-block; width: 3px; height: 20px; background-color: rgb(0, 82, 217); margin-right: 8px"></span>
        <span v-html="element.control?.modelValue" style="font-size: 16px; font-weight: 600"></span>
    ...
    <template v-else-if="element.type === 'grid'">
      <el-row class="form-row" :class="[element.config?.className]" :gutter="10">
        <el-col
    ...
    <template v-else-if="element.type === 'divider'">
      <el-divider v-bind="element.control">{{ element.control?.modelValue || element.formItem?.label }} </el-divider>
    </template>
    ...
    <template v-else-if="element.type === 'flex'">
      <form-group :data="element.list" data-type="not-nested" v-if="formProps.type === 5" />
      <flex-box :data="element" v-else />
      <el-button style="position: relative; top: -28px; left: 10px" v-if="element.config?.addBtnText && formProps.type === 5" size="small">{{
        element.config?.addBtnText
      }}</el-button>
    </template>
    <template v-else-if="element.type === 'button'">
      <div :class="[element.config?.className]">
        <el-button v-bind="element.control" @click="clickBtn(element.control)">{{ element.control?.label }}</el-button>
      </div>
    </template>
...
import { constFormBtnEvent, constFormProps, getGroupName } from '../utils/design'
...
const emits = defineEmits(['groupClick', 'changeStatus'])
 ...
 * 删除grid指定列
 */
const delGridChild = (index, columns) => {
...
const linksIf = obj => {
  ...
  const vIf = formProps.value.hideField
