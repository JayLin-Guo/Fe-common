...
<template>
  <el-form-item v-bind="data.formItem" :prop="tProp || data.name" :class="config.className" :rules="itemRules" :label="getLabel(data.formItem)">
    ...
    <template v-else>
      <el-input v-bind="control" v-model="value" :disabled="disabledEdit" :type="inputType" v-if="['input', 'password'].includes(data.type)">
        <template #prepend v-if="config.prepend">
          <div v-if="getInputSlot('p')">
            <AKSelect :data="getInputSlot('p')" :disabled="disabledEdit" :transform-option="transformOption" @change="inputSlotChange" type="slot" />
          </div>
          <span v-else>{{ config.prepend }}</span>
        </template>
        <template #append v-if="config.append">
          <div v-if="getInputSlot()">
            <AKSelect :data="getInputSlot()" :disabled="disabledEdit" :transform-option="transformOption" type="slot" @change="inputSlotChange" />
          </div>
          <span v-else>{{ config.append }}</span>
        </template>
      </el-input>
      <el-input v-bind="control" v-model="value" :disabled="disabledEdit" type="textarea" v-if="data.type === 'textarea'" />
      <el-radio-group v-bind="control" :disabled="disabledEdit" v-model="value" v-if="data.type === 'radio'">
        <el-radio :key="index" border :label="transformOption(item.value)" v-for="(item, index) in options">
          {{ item.label }}
        </el-radio>
      </el-radio-group>
      <el-checkbox-group v-bind="control" :disabled="disabledEdit" v-model="value" v-if="data.type === 'checkbox'">
        <el-checkbox v-for="(item, index) in options" :key="index" :label="transformOption(item.value)">{{ item.label }} </el-checkbox>
      </el-checkbox-group>
      <AKSelect
      ...
      <BizArea v-if="data.type === 'cityArea'" :disabled="disabledEdit" v-model="value" v-bind="control" />
      <AkTable v-if="data.type === 'table'" :data="data" :disabled="disabledEdit" v-bind="control" />
      <upload-file v-model="value" v-if="data.type === 'upload'" :control="control" :disabled="disabledEdit" :config="config" />
      ...
      <component
        ...
        v-model="value" />
      <component
        v-if="['rate', 'slider', 'switch', 'inputNumber', 'colorPicker', 'timePicker', 'datePicker', 'expand-user', 'component'].includes(data.type)"
        :is="currentComponent"
        v-bind="control"
        :disabled="disabledEdit"
        v-model="value" />
...
import { formatNumber, objectToArray, constControlChange, constSetFormOptions, constFormProps, constGetControlByName } from '../utils/design'
import validate from './validate'
import ExpandUser from './expand/user.vue'
import AKSelect from './select.vue'
// import AkArea from "./area/index.vue";
import BizArea from '@/components/BizArea/index.vue'
import AkTable from './table/index.vue'
...
const props = defineProps({
  data: { type: Object, required: true }, // FormList
  modelValue: null, // 子表和弹性布局时时有传
...
const itemRules = computed(() => {
  let temp
  const itemR = props.data.formItem?.rules || []
