...
export function json2string(obj, isBeautify) {
  return isBeautify ? JSON.stringify(obj, null, 2) : JSON.stringify(obj);
}
 ...
 * @param val
 */
export const formatToString = (val) => {
  if (val !== undefined) {
    return val.toString();
  } else {
    return val;
 ...
 */
export const appendOrRemoveStyle = (id, cssContent, append) => {
  const styleId = document.getElementById(id);
  if (styleId && append) {
    // 存在时直接修改，不用多次插入
 ...
 */
export const getGroupName = (item, index) => {
  if (item.name) {
    return item.name;
  } else {
